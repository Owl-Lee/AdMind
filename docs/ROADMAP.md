...
- **v0.5.0 released:** `/regression/intake` validates a complete schema-v2 export locally, creates an in-memory reviewed-manifest preview and re-scores the saved v0.4.0 raw predictions. It does not upload, train, commit or overwrite the tracked manifest, and the comparison is label-only rather than fresh inference. The page cannot finish Stage 1B by itself: the product owner still owes 8/8 coordinate decisions, 3/3 placement resolutions and a later review of the other seven frames.
- **1C browser reproducibility complete:** all six MediaPipe Tasks Vision 1.0.1 runtime files are self-hosted under `/mediapipe/wasm` with checksums and `s2-vision-v5` provenance. Release commit `3025d0ab4fdea704e77d01bfd122ec54e8853d40` is on public `main`; Actions run `32555440933` passed `quality` and `s2-browser-regression`, and Sites v67 is live. Hosted Playwright passed 3/3 suites covering fresh 20-frame local inference, bilingual responsive behavior at 360/430/768/1440 CSS pixels and complete browser-local schema-v2 file-intake/hash validation. `charge-005/008/013/016/018` remain temporarily diagnostic until schema-v2 intake resolves their first-pass label/box adjustments; `charge-002` is confirmed and stays in the stable gate. Outside that exception set, stable labels may not become newly unsafe. A pause-session token prevents a late vision promise from delivering after resume, seek, focus/visibility loss or reset.
...
- **Release governance:** strict branch protection requires `quality` and `s2-browser-regression`; administrator enforcement remains disabled. Completing 1C does not publish a v5 model result: the v0.4.0/v4 numbers remain historical, and holdout outcomes stay sealed until a candidate is frozen and a separate product-review artifact exists.
...
- Earlier portfolio versions include release notes, tags and GitHub Releases. The v0.5.0 annotated tag/Release remains deliberately deferred under Phase 1 release governance until the historical-media and author-email decisions are resolved.
...
- **v0.5.0 已发布：** `/regression/intake` 已能在本地校验完整 schema v2 导出、生成内存中的复核 manifest 预览，并使用 v0.4.0 已保存原始预测做重评分。页面不会上传、训练、提交或覆盖受追踪 manifest；前后对比只是标签重评分，不是新推理。该页面不能自行完成阶段 1B：产品负责人仍需完成 8/8 张坐标决定、3/3 处位置裁决，并在后续复核另外 7 张。
- **1C 浏览器可复现性工程已完成：** MediaPipe Tasks Vision 1.0.1 的 6 个 runtime 文件已带校验值固定到 `/mediapipe/wasm`，运行来源标记为 `s2-vision-v5`。发布提交 `3025d0ab4fdea704e77d01bfd122ec54e8853d40` 已进入公开 `main`；Actions 运行 `32555440933` 的 `quality` 与 `s2-browser-regression` 双绿，Sites v67 已上线。线上 Playwright 3/3 通过，覆盖 20 张本地新鲜推理、360/430/768/1440 CSS 像素双语响应式和完整的浏览器本地 schema v2 文件接收/哈希校验。`charge-005/008/013/016/018` 在 schema v2 接收解决第一轮标签/保护框调整前继续暂作诊断例外；`charge-002` 已确认正确，继续进入稳定门。除此之外，稳定标签不得新增危险误投。pause session token 会阻止迟到的视觉 Promise 在恢复、拖动、失焦/隐藏或重置后继续投放。
...
- **发布治理：** 分支保护以 strict 模式要求 `quality` 与 `s2-browser-regression`，但管理员强制执行仍关闭。完成 1C 不等于发布 v5 模型结果：v0.4.0/v4 数字继续保留为历史证据；候选冻结并形成单独产品复核文件前，不得查看或使用 holdout 结果。
...
- 早期作品集版本已经发布标签、GitHub Release 与发布说明；v0.5.0 annotated tag / Release 按阶段 1 发布治理要求暂缓，等待历史素材与作者邮箱决策完成。
- 已运行密钥、依赖与仓库体积检查。
