...
Public v0.5.0 adds `/regression/intake`, a bilingual local intake, preview and label-only re-score workspace for the future schema-v2 export. It strictly validates the source binding and required 8/8 coordinate plus 3/3 placement decisions, reuses the saved v0.4.0 raw predictions, and never uploads or overwrites the tracked manifest. The release self-hosts all six MediaPipe Tasks Vision 1.0.1 runtime files as `s2-vision-v5`, runs a fresh 20-frame Playwright Chromium CI gate, and rejects late pause-analysis promises through a session token. The gate temporarily treats `charge-005/008/013/016/018` as diagnostic because first-pass review requires their labels/boxes to be resolved; `charge-002` was confirmed correct and stays in the stable gate. Every other stable-label sample is blocked from becoming newly unsafe.

Release commit [`3025d0ab4fdea704e77d01bfd122ec54e8853d40`](https://github.com/Owl-Lee/AdMind/commit/3025d0ab4fdea704e77d01bfd122ec54e8853d40) is on public `main`. GitHub Actions [run 32555440933](https://github.com/Owl-Lee/AdMind/actions/runs/32555440933) passed both `quality` and `s2-browser-regression`; Sites deployment v67 is live at the URL above. Post-deployment Playwright passed 3/3 suites, covering fresh 20-frame local MediaPipe inference, complete bilingual responsive checks at 360/430/768/1440 CSS pixels, and the full browser-local schema-v2 file-intake/hash-validation flow. Branch protection strictly requires both checks before merge; administrator enforcement remains disabled. This completes the Stage 1C browser-reproducibility engineering gate, but it does not create or publish a new model metric.
...
- Public v0.5.0 adds a local `/regression/intake` validator and preview, self-hosted MediaPipe 1.0.1 runtime provenance as `s2-vision-v5`, an independent fresh-browser CI job and stale-promise protection for pause sessions. The intake comparison re-scores the same saved v0.4.0 raw predictions; it is not new inference. The release CI and hosted fresh runs are green, so Stage 1C browser reproducibility is complete; no new v5 model metric is produced or published. Stage 1B still requires the product owner's 8/8 coordinate decisions, 3/3 placement resolutions and later review of the other seven frames.
...
公开 v0.5.0 新增 `/regression/intake` 双语本地接收、预览与标签重评分工作区。它会严格验证来源绑定以及 8/8 张坐标决定、3/3 处位置裁决，复用 v0.4.0 已保存原始预测，绝不会上传或覆盖受追踪 manifest。该版本把 MediaPipe Tasks Vision 1.0.1 的 6 个 runtime 文件固定到站内并记为 `s2-vision-v5`，运行 20 张固定帧的 Playwright Chromium 新鲜推理 CI，并用 pause session token 拒绝迟到的暂停分析 Promise。安全门会把仍需解决标签/保护框的 `charge-005/008/013/016/018` 暂作诊断例外；`charge-002` 已确认正确，继续进入稳定门。除此之外，稳定标签不得新增危险误投。

发布提交 [`3025d0ab4fdea704e77d01bfd122ec54e8853d40`](https://github.com/Owl-Lee/AdMind/commit/3025d0ab4fdea704e77d01bfd122ec54e8853d40) 已进入公开 `main`。GitHub Actions [运行 32555440933](https://github.com/Owl-Lee/AdMind/actions/runs/32555440933) 的 `quality` 与 `s2-browser-regression` 双绿；Sites v67 已部署到上方公开地址。部署后 Playwright 3/3 通过，覆盖 20 张本地 MediaPipe 新鲜推理、360/430/768/1440 CSS 像素下的完整双语响应式，以及完整的浏览器本地 schema v2 文件接收/哈希校验流程。分支保护以 strict 模式要求两个检查后才能合并，但管理员强制执行仍关闭。阶段 1C 的浏览器可复现性工程因此完成；这不会产生或发布新的模型指标。
...
- 公开 v0.5.0 新增本地 `/regression/intake` 校验与预览、`s2-vision-v5` 的 MediaPipe 1.0.1 本地运行时、独立新鲜浏览器 CI，以及暂停会话的迟到 Promise 防护。接收页前后对比使用同一份 v0.4.0 已保存原始预测，只是标签重评分，不是新推理。发布 CI 与线上新鲜运行已通过，阶段 1C 的浏览器可复现性工程已完成；没有产生或发布新的 v5 模型指标。阶段 1B 仍需产品负责人完成 8/8 张坐标决定、3/3 处位置裁决，并在后续复核另外 7 张。
